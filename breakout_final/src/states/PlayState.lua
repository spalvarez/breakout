...
function PlayState:update(dt)
    ...
    --check to see if a multi-ball powerup was hit
    if HAS_EXTRA_BALLS_POWERUP then
        HAS_EXTRA_BALLS_POWERUP = false
        --spawn 2 new balls
        self.numBalls = self.numBalls + 2

        self.ball[#(self.ball)+1] = Ball()
        self.ball[#(self.ball)].dx = math.random(-200, 200)
        self.ball[#(self.ball)].dy = math.random(-50, -60)
        self.ball[#(self.ball)].skin = math.random(7)
        self.ball[#(self.ball)].x = self.paddle.x + (self.paddle.width / 2) - 4
        self.ball[#(self.ball)].y = self.paddle.y - 8
    ...
    for k, brick in pairs(self.bricks) do
        ...
        for i=1, #(self.ball) do
            if brick.inPlay and self.ball[i].inPlay and self.ball[i]:collides(brick) then
    ...
    -- if ball goes below bounds, mark ity out of play
    for i=1,#(self.ball) do
        if self.ball[i].inPlay and self.ball[i].y >= VIRTUAL_HEIGHT then
            self.numBalls = self.numBalls -1
            self.ball[i].inPlay = false
        end
    end

    --if we've run out of balls, decrease health
    if self.numBalls == 0 then
